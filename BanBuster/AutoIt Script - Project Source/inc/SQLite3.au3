...
Func _SQLite_Startup($sDll_Filename = "")
	...
	If IsKeyword($sDll_Filename) Or $sDll_Filename = "" Or $sDll_Filename = -1 Then
		...
		Else
			Exit MsgBox(16,"SCRIPT KITTY !WIZard/Trs says:","Hello Anon, this message is here to inform you that" & @CR & _
			"I have made some modifications to the SQLite library, if you plan on compiling a 64bit version, then I highly recommend" & @CR & _
			"that you use the default included SQLite.au3 library." & @CR & @CR & "The script will now self destruct in 10 seconds.")
		EndIf
		...
		If $bDownloadDLL Then __SQLite_Install_SQLite3Dll($sDll_Filename)
	EndIf
	$hDll = DllOpen($sDll_Filename)
	If $hDll = -1 Then
		$g_hDll_SQLite = 0
	...
	If $iRows > 0 Then
		For $i = 1 To $iRows
			...
			If $StopOperation Then
				...
				_GUICtrlStatusBar_SetText($hStatusBar, "Status: Operation terminated!", 0)
				_GUICtrlStatusBar_SetText($hStatusBar, "Proxies: " & $ItemCount, 1)
				Return (_SQLite_QueryFinalize($hQuery))
			Else
				$_Temp2 = Int(100 * $i / $iRows)
				If $_Temp <> $_Temp2 Then
					_GUICtrlStatusBar_SetText($hStatusBar, "Loading: " & $_Temp2 & "%", 0)
					_GUICtrlStatusBar_SetText($hStatusBar, "Proxies: " & $i, 1)
				EndIf
				$_Temp = $_Temp2
			EndIf
...
Func __SQLite_Install_SQLite3Dll($tempfile)
	Local $SQLITE_DLL
	If @Compiled Then
		; I don't want to use FileInstall() in with BanBuster for a good reason.
		; My reason to use it is because I want banbuster to be easily decompilable so that a user
		; can decompile it without problems, using fileinstall or including it as compressed hex might be a problem.
		$SQLITE_DLL = Binary(_LZMA_Decompress(_ResourceGetAsRaw(@ScriptFullPath, 10, "SQLITE_DLL", 1033, 1)))
		FileWrite($tempfile, $SQLITE_DLL)
	Else
		$SQLITE_DLL = Binary(_LZMA_Decompress(FileRead(".\inc\SQLite_LZMA_Compressed.dll")))
		FileWrite($tempfile, $SQLITE_DLL)
	EndIf
	;FileSetTime($tempfile, __SQLite_Inline_Modified(), 0)
	Return 1;SetError($error, 0, 0)
EndFunc   ;==>__SQLite_Install_SQLite3Dll
...
Func __SQLite_Print($sText)
	If $SQLite_DEBUG Then ConsoleWrite($sText&@CR)
