...
Global $___nOffset = 0 ;No. of spaces crossed
...
; Syntax.........: _RegisterPrediction($Edit_Handle[, $_Predict_List = ''[, $Sensitive = 2[, $SpacePredict = 0[, $_AddNewWords = 0[,  $MaxNwWrds = 1000]]]]])
;
...
Func _RegisterPrediction($Edit_Handle, $_Predict_List = '', $Sensitive = 2, $SpacePredict = 0, $_AddNewWords = 0)
	...
	If $_Predict_List = Default Then $_Predict_List = ''
...
;							| 2 - The New word will make the Array Out of Bounds
;								| @extended is set to the Element Number of the Requested word
;
; Author ........: Phoenix XL
; Modified.......:
...
; Syntax.........: _RegisterListingNewWords([$_wFlag = -1[, $Min_Len = 3]])
...
; Syntax.........: _GetCurrentWord($Edit_ID[, $_CaretIndex = -1])
...
; Return values .: Success		- Returns the Word & Sets @extended to
;						| -1 - The Returned Word is  Blank('') or Space(' ')
;						| Integer - The Character Offset of the Last Alphabet of the Word.
;				   Failure		- Returns -1 & Sets @error to 1
;
; Author ........: Phoenix XL
; Modified.......:
...
Func _GetCurrentWord($Edit_ID, $_CaretIndex = -10, $cEnter = 0)
	Local $Edit_Handle = GUICtrlGetHandle($Edit_ID)
	If $_CaretIndex = Default Then $_CaretIndex = -1
	Local $_LineIndex
		...
		Switch $_CaretIndex
			Case $_Offset To $_Offset + StringLen($_SpaceSplit[$x])
				Switch $_SpaceSplit[$x]
					Case '', ' '
						Return SetError(0, -1, $_SpaceSplit[$x])
					Case Else
						Return SetExtended($_Offset + StringLen($_SpaceSplit[$x]), $_SpaceSplit[$x])
				EndSwitch
		EndSwitch
...
Func _GetCaretOffset($Edit_ID)
	...
	Switch $_LineIndex[0]
		...
		Case Else
			...
			Return SetExtended(1, $_LineIndex[BitAND($_Caret[0] = $_TextPos[0], $_Caret[1], $_TextPos[1])])
	EndSwitch
...
Func _New_WndProc($hWnd, $iMsg, $wParam, $lParam)
	...
	Local $_hWnd = _WinAPI_GetFocus()
	Local $cOffset, $_NewWord
	If $_hWnd <> $___cEdit Then Return _WinAPI_CallWindowProc($___pOld_WndProc, $hWnd, $iMsg, $wParam, $lParam)
	Local $IDFrom = _WinAPI_GetDlgCtrlID($_hWnd)
	Switch $iMsg
				...
				Case 0x0D ;Enter
					...
					Switch $nExtended
						Case 0 ;Nothing Selected
							If BitAND(_CtrlGetStyle($_hWnd), $ES_MULTILINE) Then _GUICtrlEdit_InsertText($_hWnd, @CRLF, $cOffset) ;Send @CRLF to a MultiLine Control
							...
							Switch $___iNewWords
								...
								Case 1
									...
									If BitAND(_CtrlGetStyle($_hWnd), $ES_MULTILINE) Then $MultiLine = 1
									$_NewWord = _GetCurrentWord($IDFrom, $cOffset - 2, $MultiLine)
									If StringLen($_NewWord) < $___Min_Count Then Return _WinAPI_CallWindowProc($___pOld_WndProc, $hWnd, $iMsg, $wParam, $lParam)
						...
						Case 1 ;Something is Selected
							_GUICtrlEdit_SetSel($_hWnd, -1, -1) ;Deselect Selection
					EndSwitch
				Case 0x20 ;Spacebar
					Switch $___iNewWords
						Case 0
									...
									Case 1
										For $i = 0 To UBound($___nList) - 1
											If $_NewWord = $___nList[$i] Then
												ExitLoop
											Else
												If $i = UBound($___nList) - 1 Then AddToArray($___nList, $_NewWord, 1)
											EndIf
										Next
									Case 0
										Local $_MakeArray[1] = [$_NewWord]
					...
					EndSwitch
				Case 0x08 ;BackSpace
					_GUICtrlEdit_ReplaceSel($_hWnd, '')
			EndSwitch
		Case 0x0102 ;WM_CHAR
			Local $aRet
			Switch $wParam
				Case 33 To 65535 ;Printing Chars [Exclusive of Space and DEL]
					$aRet = _WinAPI_CallWindowProc($___pOld_WndProc, $hWnd, $iMsg, $wParam, $lParam)
					_PredictText($IDFrom)
					Return $aRet
				Case 32
					Switch $___SpaceNext
...
Func _PredictText($Edit_ID)
	...
	Local $sMatch
	For $i = 0 To UBound($___nList) - 1
		$sMatch = _MatchString($___nList[$i], $sRead)
		If $sMatch Then
			Return _SetSelection($Edit_ID, $___nList[$i], $sMatch + StringLen($sRead))
		EndIf
	Next
...
; Example .......:
; ===============================================================================================================================
Func _PredictSpaceText($Edit_ID, $cWord = -1, $nExtended = -1)
	If $cWord = -1 Then
		$cWord = _GetCurrentWord($Edit_ID)
		$nExtended = @extended
	EndIf
	Local $pWord = _GetCurrentWord($Edit_ID, $nExtended - StringLen($cWord) - 2)
	If $pWord < 0 Then Return 0
	Local $sWord = $pWord & ' ' & $cWord
	Local $Match
	$___nOffset += 1
	For $i = 0 To UBound($___nList) - 1
		$Match = _MatchString($___nList[$i], $sWord)
		If $Match Then
			$___nOffset = 0
			Return _SetSelection($Edit_ID, $___nList[$i], StringLen($sWord) + $Match)
		EndIf
	Next
	If $___nOffset < $___nSpaceRecurse Then Return _PredictSpaceText($Edit_ID, $sWord, $nExtended)
	Return 0
